//! Zed bridge — opens the project directory in Zed editor.
//!
//! Launch mechanism: `zed <project_path>` if the CLI is on PATH,
//! otherwise `open -a Zed <project_path>`.

use tokio::process::Command;

use crate::agent_loop::tools::dev_tools::discover::{discover_zed, CapabilityReport};
...
pub fn build_launch_cmd(project_path: &str, use_cli: bool) -> Vec<String> {
    if use_cli {
        vec!["zed".to_string(), project_path.to_string()]
    } else {
        vec![
            "open".to_string(),
            "-a".to_string(),
...
/// Returns true if `zed` is on PATH.
fn zed_cli_available() -> bool {
...
pub async fn launch(project_path: &str, session_id: &str) -> Result<(), String> {
    ...
    let status = Command::new(&cmd[0])
        ...
        .status()
        .await
        .map_err(|e| format!("zed launch failed: {e}"))?;

    if status.success() {
        write_bus_status(session_id, "running")?;
        Ok(())
    } else {
        let err = format!("zed launch exited with {status}");
